...
l'analisis statica di codice, ovvero riuscire a individuare a /compile time/
...
Il lavoro svolto è stato quello di, inizialmente, effettuare un porting della
/fattorizzazione cartesiana/ dalla PPL, descritta in \cite{Arioli17th}, alla
PPLite. Una volta effettuato il porting si è provveduto a modernizzare il codice
...
In questa sezione verranno mostrati i diversi test di correttezza che sono stati
effettuati e un test di efficienza per palesare l'effettivo guadagno che ci si
aspettava.
...
qualvolta viene costruito un oggetto di tipo ~Poly~ ne viene costruito, in
realtà, uno di tipo ~F_Poly~. Per l'esattezza, sono stati completati con
successo 143 test di correttezza, tra cui:
...
dei benchmark di /PHAVerLite/. Sono stati generati due poliedri di tipo ~Poly~ di dimensioni elevate,
successivamente è stato applicato il join ($\bowtie$) tra questi due, in seguito
sono stati costruiti due oggetti di tipo ~F_Poly~ passando ai costruttori i due
poliedri di cui sopra e applicato il join, infine è stata applicata una funzione
(resa disponibile dagli sviluppatori della PPLite) per convertire i poliedri
fattorizzati in oggetti di tipo ~Poly~.

I risultati sono stati questi:
* Conclusione
In questa tesi si è quindi parlato della fattorizzazione dei poliedri, ovvero la
tecnica di suddividere un poliedri in poliedri più piccoli, ricudendo così il
...
in modo tale da rendere completa classe. Inoltre, in quanto non ci si è
concentrati principalmente nell'efficienza del codice, sarà necessario rivederlo
andando alla ricerca di punti critici per ottimizzare il codice e sfruttare al
meglio le funzionalità offerde dal C++11.

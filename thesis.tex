...
\hyphenation{rap-pre-se-nta-re}
\hyphenation{o-pe-ra-zio-ni}
\hyphenation{o-pe-ra-zio-ne}
\hyphenation{com-ples-si-ta}
\hyphenation{i-ni-zial-men-te}
\hyphenation{pa-ra-me-tri}
\usepackage{listings}
\usepackage{color}
...
l'analisi statica di codice, ad esempio per individuare a \emph{compile time}
...
poliedri. Un esempio è la rappresentazione \emph{singola} (che utilizza solamente vincoli) o la \emph{doppia
...
\emph{fattorizzazione cartesiana} dalla PPL, descritta in \cite{Arioli17th}, alla
...
\begin{itemize}
\item nel Capitolo 1 verranno introdotte, formalmente, le funzioni principali
descritte in \cite{SinghPV17};
\item nel Capitolo 2 verranno descritte le implementazioni concrete di suddette
funzioni e anche di alcune funzioni helper e aggiuntive;
\item nel Capitolo 3 verranno mostrati alcuni test di correttezza e un test di
efficienza;
\item nella conclusione verrà discusso il lavoro svolto e proposti sviluppi futuri.
\end{itemize}

\chapter{Fattorizzazione cartesiana}
\label{sec:org3c1b52b}
...
\label{sec:orgbcb473c}
Una partizione \(\pi\) è \emph{ammissibile} per un poliedro \(\mathcal{P}\) se non esistono
...
\emph{wrapper} preesistente nella PPLite mentre alcune delle implementazioni sono
...
\begin{enumerate}
\item inizialmente è stato effettuato un porting del codice dalla PPL alla
...
\label{sec:orgdf0244a}
La semantica rimane invariata invariata rispetto a quanto descritto
nell'Algorit\-mo \ref{refactor}. Per la \emph{join} \((\bowtie)\) tra i fattori è stata
...
inizialmente, crea un blocco \texttt{lub} che sarà composto dalle dimensioni in
...
richiamare la funzione \texttt{contains} della PPLite.
...
\label{sec:org2a36b8e}
La funzione \texttt{affine\_image} è l'implementazione dell'operazione \emph{assignment} vis\-ta
...
\end{lstlisting}
È presente anche \texttt{remove\_space\_dims(const Index\_Set\& vars)} che
...
qualvolta viene costruito un oggetto di tipo \texttt{Poly} ne venisse costruito, in
realtà, uno di tipo \texttt{F\_Poly}. Per l'esattezza, sono stati completati con
